...
class LaporanController extends Controller
{
    ...
    public function status()
    {
        ...
        $mhs   = Mahasiswa::where('Nim', (int) $user->username)->first();

        $laporan = $mhs
            ? Laporan::with(['kategori', 'subkategori', 'lokasi'])
    ...
    public function store(Request $request)
    {
        ...
        // Jika data mahasiswa tidak ditemukan, tolak pengiriman laporan
        if (!$mhs) {
            return back()->withErrors([
                'general' => 'Data mahasiswa Anda tidak ditemukan. Silakan lengkapi biodata terlebih dahulu atau hubungi admin.',
            ])->withInput();
        }
